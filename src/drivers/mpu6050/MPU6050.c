...
uint8_t mpu6050_begin(struct mpu6050 *self)
{
    ...
    mpu6050_set_sleep_enabled(self, 0);
    sleep_ms(100);  // Wait for device to wake up
    
    uint8_t who_am_i = mpu6050_who_am_i(self);
    
    // MPU6050 variants return different WHO_AM_I values:
    // 0x68 = MPU6050, 0x70 = MPU6500, 0x71 = MPU9250, 0x72 = ICM-20608
    // Also check for bit 6 which identifies MPU60X0/MPU65XX family (0x68-0x72)
    uint8_t who_am_i_masked = who_am_i & 0x7E;  // Mask bit 0
    if (who_am_i_masked != 0x68 && who_am_i != 0x70 && who_am_i != 0x71 && who_am_i != 0x72) 
    {
        return 0;
    }

    mpu6050_set_clock_source(self, MPU6050_CLOCK_PLL_XGYRO); // Use PLL with X gyro reference
    sleep_ms(10);
