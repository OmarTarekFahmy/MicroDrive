...
void servo_set_angle(uint8_t servo_id, float angle) {
    ...
    if (s->servo_type == SERVO_TYPE_180) {
        ...
        // Very small dead zone for high sensitivity
        if (fabsf(angle_diff) < 0.2f) {
            set_pulse_width(servo_id, SERVO_CENTER_US);  // Stop
            s->current_angle = angle;
            return;
        }
        
        // Speed proportional to error: full speed at 30° diff (more aggressive)
        float speed = angle_diff / 30.0f;
        if (speed > 1.0f) speed = 1.0f;
        if (speed < -1.0f) speed = -1.0f;
        
        // Lower minimum speed for more sensitivity
        if (speed > 0 && speed < 0.10f) speed = 0.10f;
        if (speed < 0 && speed > -0.10f) speed = -0.10f;
